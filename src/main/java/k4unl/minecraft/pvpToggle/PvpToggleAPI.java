...
public class PvpToggleAPI implements IPvpToggleApi {
    ...
    @Override
    public void setDimensionSetting(DimensionType dimension, PvPStatus toSet) throws IncorrectStatusException {

        if (toSet == PvPStatus.OFF || toSet == PvPStatus.ON) {
            throw new IncorrectStatusException();
        }
        ServerHandler.getDimensionSettings().remove(dimension);
        ServerHandler.getDimensionSettings().put(dimension, toSet);
    }
    ...
    @Override
    public void createNewArea(String name, BlockPos location1, BlockPos location2, DimensionType dimension, PvPStatus toSet, boolean announce) throws AreaNameAlreadyExistsException, IncorrectStatusException {
        ...
        PvPArea newArea = new PvPArea(name, loc1, loc2, dimension);
    ...
    @Override
    public DimensionType getDimensionFromArea(String areaName) throws NoAreaFoundException {
        PvPArea area = Areas.getAreaByName(areaName);
        if (area == null) {
            throw new NoAreaFoundException();
        } else {
