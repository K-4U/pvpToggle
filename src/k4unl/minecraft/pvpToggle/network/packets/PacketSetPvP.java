...
public class PacketSetPvP extends AbstractPacket<PacketSetPvP> {
    ...
    public void handleClientSide(PacketSetPvP message, EntityPlayer player) {
        ...
        if (ClientHandler.getClientPvPStatus().containsKey(message.user)) {
            ClientHandler.getClientPvPStatus().remove(message.user);
        }
        ClientHandler.getClientPvPStatus().put(message.user, message.pvpStatus);
    }
